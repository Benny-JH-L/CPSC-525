...
const char * password = QUOTE(SECRET);
    ...
    void * is_correct_addr)
{
    ...
    printf("DEBUG: $1 = %p\n", buff_addr);
    printf("DEBUG: (gdb) print/x &is_correct\n");
    printf("DEBUG: $2 = %p\n", is_correct_addr);
...
int main()
{
    int debug = getenv("2+2is5") != NULL;

    int is_correct = 0;
    char buff[20];
    printf("---is_correct: %ld", is_correct);   // mine
    ...
    while (is_correct == 0) {
        get_pass(buff);
        if (strlen(buff) == 0) {
            printf("empty password, try again\n");
            printf("---is_correct: %ld\n", is_correct);   // mine
            printf("---buff: %s | password: %s\n", buff, password);   // mine
            continue;
        }
